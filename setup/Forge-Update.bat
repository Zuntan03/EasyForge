...
@REM config.json での t5xxl-Q8_0.gguf デフォルト化に伴う特例 DL
if exist "%~dp0..\stable-diffusion-webui-forge\models\VAE\Flux\clip_l.safetensors" (
	if not exist "%~dp0..\stable-diffusion-webui-forge\models\VAE\Flux\t5xxl-Q8_0.gguf" (
		call "%~dp0..\download\VAE\Flux\t5xxl-Q8_0.bat"
	)
)
